 ...
 *
 * JavaScript's Date constructor doesn't reliably parse timezone
 * abbreviations like "AEST". This map provides the UTC offsets
 * for common Australian timezones.
 */
const TIMEZONE_OFFSETS: Record<string, string> = {
  ...
  UTC: '+00:00',
  GMT: '+00:00',
  EST: '-05:00', // US Eastern Standard Time
  EDT: '-04:00', // US Eastern Daylight Time
  PST: '-08:00', // US Pacific Standard Time
  PDT: '-07:00', // US Pacific Daylight Time
};

/**
 * Parses a journal date string with timezone abbreviation
 *
 * Handles formats like "2026-01-02 21:41 AEST" by converting
 * timezone abbreviations to proper UTC offsets.
 *
 * @param dateStr - Date string from journal entry (e.g., "2026-01-02 21:41 AEST")
 * @returns ISO 8601 timestamp or current time if parsing fails
 */
export function parseJournalDate(dateStr: string): string {
  // Try to match format: YYYY-MM-DD HH:MM TZ
  const match = dateStr.match(/^(\d{4}-\d{2}-\d{2})\s+(\d{2}:\d{2})\s+(\w+)$/);

  if (match) {
    const [, datePart, timePart, tzAbbr] = match;
    const offset = TIMEZONE_OFFSETS[tzAbbr.toUpperCase()];

    if (offset) {
      // Construct ISO 8601 format: YYYY-MM-DDTHH:MM:SS±HH:MM
      const isoString = `${datePart}T${timePart}:00${offset}`;
  ...
  // Fallback: try native Date parsing (works for ISO 8601 strings)
  ...
  console.warn(`  Warning: Could not parse date "${dateStr}", using current time`);
...
export async function seedExistingJournal(
  ...
  try {
    ...
    const { extractTopics, createCondensedSummary, extractWorkCompleted, extractOpenItems } =
      await import('./journal-generator.js');
    ...
    for (const entry of entries) {
      try {
        ...
        const summary =
          summaryIdx >= 0 && summaryIdx + 1 < lines.length
            ? lines
        ...
        // Use parseJournalDate to handle timezone abbreviations like AEST/AEDT
...
export async function seedAll(paths?: {
  ...
  console.log(
    `Journal: ${journalResult.success ? 'OK' : 'FAILED'} (${journalResult.count} entries)`
  );
